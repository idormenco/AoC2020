...
let problemInput = @"plaid beige bags contain 3 drab magenta bags.
...
type Innerbag = { Color: string; Count: int }

type Bag =
    { Color: string
      Content: Innerbag list }


let parseColor (row: string) =
    let splitResult =
        row.Split([| " bags " |], StringSplitOptions.None)

    splitResult.[0]

let parseContent (row: string): Innerbag list =
    let content =
        row.Split([| " contain " |], StringSplitOptions.None).[1]

    match content with
    | _ when content = "no other bags." -> []
    | bags ->
        bags.Replace(".", "").Split(',')
...
let parseBags (text: string): Bag list =
    let rows = text.Split('\n') |> Seq.toList

    let rec loop rows =
        match rows with
        | row :: rest ->
            { Color = (parseColor row)
...
let getBagsContaining (bags: Bag list) (bag: string) =
    bags
    |> List.map (fun b ->
        { b with
              Content =
                  (b.Content
                   |> List.filter (fun ib -> ib.Color = bag)) })
    |> List.filter (fun b -> (not (List.isEmpty b.Content)))
    |> List.map (fun b -> b.Color)

let getBag (bag: string) (bags: Bag list) =
    let foundBags =
        bags |> List.filter (fun b -> b.Color = bag)

    match foundBags with
    | [] -> failwith ("no bags found")
    | [ bag ] -> bag
    | h :: t -> failwith ("multiple bags found")

let getBagParrents bags lst =
    lst
    |> List.map (fun bag -> getBagsContaining bags bag)
    |> List.reduce (@)


let rec getBagsContainingShinyGold bagsRules =
    let bagsContainingShinyGold = getBagsContaining bagsRules "shiny gold"

    let rec loop bagsRules lst nr =
        match lst with
        | [] -> nr
        | list ->
            let parrents = getBagParrents bagsRules lst
            loop bagsRules parrents (nr @ parrents)

    loop bagsRules bagsContainingShinyGold bagsContainingShinyGold

let bagsRules = parseBags problemInput

let resultQuestion1 =
    getBagsContainingShinyGold bagsRules
    |> Set.ofList
    |> Set.count

let shinyGoldBag = bagsRules |> getBag "shiny gold"
