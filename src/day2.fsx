...
let isPasswordValid password character minOccurrence maxOccurrence =
    match password
          |> Seq.toList
          |> List.filter (fun c -> c = character)
          |> List.length with
    | x when (x >= minOccurrence && x <= maxOccurrence) -> true
    | _ -> false

let isPasswordValidNewRules (password: string) (character: char) (position1: int) (position2: int) =
    match password |> Seq.toArray with
    | charArray when (charArray.[position1 - 1] = character
                      && charArray.[position2 - 1] <> character) -> true
    | charArray when (charArray.[position1 - 1] <> character
                      && charArray.[position2 - 1] = character) -> true
    | _ -> false

let isNumber (value: string) =
...
let parseInput (input: string) =
    ...
    | [] -> None
    | [ minOccurrence; maxOccurrence; character; password ] when isNumber minOccurrence && isNumber maxOccurrence ->
        Some(minOccurrence |> int, maxOccurrence |> int, character.[0], password)
    | _ -> None


problemInput
|> Seq.map (fun s -> parseInput s)
|> Seq.choose id
|> Seq.map (fun (min, max, character, password) -> isPasswordValidNewRules password character min max)
