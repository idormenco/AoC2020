...
let parsePassport (passportFields: string list) =
    let parsePassword =
        { Byr = ""
          Iyr = ""
          Eyr = ""
    ...
    let rec loop (fields: string list) (accumulator: ParsedPassword) =
        match fields with
        | field :: tail ->
            match field.Split(':') with
            | [| "byr"; byr |] -> loop tail { accumulator with Byr = byr }
            | [| "iyr"; iyr |] -> loop tail { accumulator with Iyr = iyr }
            | [| "eyr"; eyr |] -> loop tail { accumulator with Eyr = eyr }
            | [| "hgt"; hgt |] -> loop tail { accumulator with Hgt = hgt }
            | [| "hcl"; hcl |] -> loop tail { accumulator with Hcl = hcl }
            | [| "ecl"; ecl |] -> loop tail { accumulator with Ecl = ecl }
            | [| "pid"; pid |] -> loop tail { accumulator with Pid = pid }
            | [| "cid"; cid |] -> loop tail { accumulator with Cid = cid }
            | _ -> accumulator
        | [] -> accumulator

    loop passportFields parsePassword

let tryParseNumber (value: string) =
    match value |> System.Int32.TryParse with
    | true, v -> Some v
    | false, _ -> None

let (|HasInvalidByr|Valid|) (passport: ParsedPassword) =
    if passport.Byr = "" then
        HasInvalidByr
    else
        match tryParseNumber passport.Byr with
        | Some value when value < 1920 || value > 2002 -> HasInvalidByr
        | _ -> Valid

let (|HasInvalidIyr|Valid|) (passport: ParsedPassword) =
    if passport.Iyr = "" then
        HasInvalidIyr
    else
        match tryParseNumber passport.Iyr with
        | Some value when value < 2010 || value > 2020 -> HasInvalidIyr
        | _ -> Valid

let (|HasInvalidEyr|Valid|) (passport: ParsedPassword) =
    if passport.Eyr = "" then
        HasInvalidEyr
    else
        match tryParseNumber passport.Eyr with
        | Some value when value < 2020 || value > 2030 -> HasInvalidEyr
        | _ -> Valid

let (|HasInvalidImpericHeight|Valid|) (height: string) =
    let inches = height.Replace("in", "")
    match tryParseNumber inches with
    | None -> HasInvalidImpericHeight
    | Some value when value < 59 || value > 76 -> HasInvalidImpericHeight
    | _ -> Valid

let (|HasInvalidMetricHeight|Valid|) (height: string) =
    let cm = height.Replace("cm", "")
    match tryParseNumber cm with
    | None -> HasInvalidMetricHeight
    | Some value when value < 150 || value > 193 -> HasInvalidMetricHeight
    | _ -> Valid


let (|HasInvalidHgt|Valid|) (passport: ParsedPassword) =
    if passport.Hgt = "" then
        HasInvalidHgt
    else
        match passport.Hgt with
        | cm when cm.Contains("cm") ->
            match cm with
            | HasInvalidMetricHeight -> HasInvalidHgt
            | _ -> Valid
        | inches when inches.Contains("in") ->
            match inches with
            | HasInvalidImpericHeight -> HasInvalidHgt
            | _ -> Valid
        | _ -> HasInvalidHgt

let contains x = Seq.exists ((=) x)

let allCharsFromHex text =
    let allowedChars =
        [| '0'
           '1'
           '2'
    ...
    text
    |> Seq.toList
    |> Seq.map (fun x -> contains x allowedChars)
    |> Seq.filter ((=) false)
    |> Seq.length
    |> (<>) 0
...
let (|HasInvalidHcl|Valid|) (passport: ParsedPassword) =
    if passport.Hcl = "" then
        HasInvalidHcl
    else
        match passport.Hcl with
        | hex when not (hex.StartsWith("#")) -> HasInvalidHcl
        | hex when not (allCharsFromHex hex) -> HasInvalidHcl
        | _ -> Valid

let (|HasInvalidEcl|Valid|) (passport: ParsedPassword) =
    if passport.Ecl = "" then
        HasInvalidEcl
    else
...
let (|HasInvalidPid|Valid|) (passport: ParsedPassword) =
    if passport.Pid = "" then
        HasInvalidPid
    else
        match passport.Pid with
        | pid when pid.Length <> 9 -> HasInvalidPid
        | pid when (tryParseNumber pid) = None -> HasInvalidPid
        | _ -> Valid

let parsePassports (input: string) =
    let data =
        input.Replace("\r", "").Replace(" ", "\n").Split('\n')
        |> Seq.toList

    let rec loop rows passports passport =
        match rows with
        | field :: tail ->
            match field with
            | "" -> loop tail (passport :: passports) []
            | _ -> loop tail passports (field :: passport)
        | [] -> passports

    loop data [] [] |> Seq.map parsePassport

let isValidPassword (passport: ParsedPassword) =
    let result =
        match passport with
        | HasInvalidByr -> false
        | HasInvalidIyr -> false
...
let main argv =
    ...
    let result =
        testInput |> parsePassports |> Seq.toArray
